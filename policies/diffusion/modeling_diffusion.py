...
class DiffusionPolicy(PreTrainedPolicy):
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # batch = {k: torch.cat(list(self._queues[k]), dim=1) for k in batch if k in self._queues}
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Make sure the shapes for the images are the same for all n_obs_steps values
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, None]:
        ...
        batch = self.normalize_inputs(batch)
        # Fix: Make sure the shapes for the images are the same for all n_obs_steps values
        if self.config.n_obs_steps == 1:
            for key in self.config.image_features:
                batch[key] = batch[key].unsqueeze(1)
...
class DiffusionModel(nn.Module):
    ...
    def generate_actions(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Align image keys as in forward()
        if self.config.n_obs_steps == 1:
            for key in self.config.image_features:
                batch[key] = batch[key].unsqueeze(1)
...
class DiffusionConditionalUnet1d(nn.Module):
    ...
    def __init__(self, config: DiffusionPolicyConfig, global_cond_dim: int):
        super().__init__()
